...
class AdapterRVHistory(private var data: ArrayList<PermissionModel>, private var c: Context) :
    ...
    override fun onBindViewHolder(myViewHolder: MyViewHolder, i: Int) {
        ...
        when(data[i].statusId){
            1 -> v.tv_status.background.setColorFilter(ContextCompat.getColor(c, R.color.orange_pale), PorterDuff.Mode.SRC_ATOP)
            2 -> v.tv_status.background.setColorFilter(ContextCompat.getColor(c, R.color.green_pale), PorterDuff.Mode.SRC_ATOP)
            3 -> v.tv_status.background.setColorFilter(ContextCompat.getColor(c, R.color.red_pale), PorterDuff.Mode.SRC_ATOP)
        }
